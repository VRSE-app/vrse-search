export default function() {
    ...
    function solveForConstants() {
        ...
        const smoothingCoefficient =
            (2 * xw * (d * xw - Math.sqrt(Math.pow(d, 2) * Math.pow(xw, 2) + d * 
                Math.pow(xw, 2) + d + 1) +1)) / (Math.pow(xw, 3) - 3 * Math.pow(xw, 2) + 3 * xw - 1);
      
        const sarkarCoefficient =
            (
                xw *
                (Math.pow(d, 2) * xw +
                    d * Math.pow(xw, 2) + 2 * d * xw - d *
                    Math.sqrt(
        ...
        const distortionCoefficient =
            (d * xw + Math.sqrt((d + 1) * (d * Math.pow(xw, 2) + 1)) - 1) / (xw * (xw + 1));
